...
pagetable_t kvmcreate() {//step2
  ...
  // uart registers
  new_kvmmap(pagetable, UART0, UART0, PGSIZE, PTE_R | PTE_W);

  // virtio mmio disk interface
  new_kvmmap(pagetable, VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);

  // PLIC
  new_kvmmap(pagetable, PLIC, PLIC, 0x400000, PTE_R | PTE_W);

  // map kernel text executable and read-only.
  new_kvmmap(pagetable, KERNBASE, KERNBASE, (uint64)etext - KERNBASE, PTE_R | PTE_X);

  // map kernel data and the physical RAM we'll make use of.
  new_kvmmap(pagetable, (uint64)etext, (uint64)etext, PHYSTOP - (uint64)etext, PTE_R | PTE_W);

  // map the trampoline for trap entry/exit to
  // the highest virtual address in the kernel.
  new_kvmmap(pagetable, TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
...
void new_kvmmap(pagetable_t pagetable, uint64 va, uint64 pa, uint64 sz, int perm) {
  if (mappages(pagetable, va, sz, pa, perm) != 0) panic("new_kvmmap");
}
...
void user_kernel_map(pagetable_t u_pagetable,pagetable_t k_pagetable,uint64 start,uint64 end){//step 3.1
...
int copyin(pagetable_t pagetable, char *dst, uint64 srcva, uint64 len) {
  ...
  // }
  // return 0;
  return copyin_new(pagetable, dst, srcva, len);//step 3.2
}
